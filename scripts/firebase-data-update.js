// Firebase Data Update Script
// This script replaces update-stats.js, updating Firebase Realtime Database instead of generating JSON files
...
// SQL Queries (same as in update-stats.js)
const queries = {
...
async function updateSeasonAvgStats() {
  try {
    ...
    await writeToFirebase(DB_PATHS.SEASON_AVG, transformedData);
    console.log('✅ Season average stats written to Firebase');
  } catch (error) {
...
async function updateNightAvgStats() {
  try {
    ...
    // Write the result to Firebase
    await writeToFirebase(DB_PATHS.NIGHT_AVG, transformedData);
    console.log('✅ Night average stats written to Firebase');
  } catch (error) {
...
async function updateLast10Stats() {
  try {
    ...
    // Write the result to Firebase
    await writeToFirebase(DB_PATHS.LAST10, transformedData);
    console.log('✅ Last 10 match stats written to Firebase');
  } catch (error) {
...
async function updateSonMacStats() {
  try {
    ...
    if (!results || !results.rows || !results.rows.length) {
      ...
      await writeToFirebase(DB_PATHS.SON_MAC, emptyData);
      console.log('✅ Empty Son Maç stats written to Firebase');
      return;
    }
    ...
    // Write the result to Firebase
    await writeToFirebase(DB_PATHS.SON_MAC, finalData);
    console.log('✅ Son Maç stats written to Firebase');
...
async function updateDuelloSonMacStats() {
  try {
    ...
    if (!results || !results.rows || !results.rows.length) {
      ...
      await writeToFirebase(DB_PATHS.DUELLO_SON_MAC, emptyData);
      console.log('✅ Empty Duello Son Maç stats written to Firebase');
      return;
    }
    ...
    // Write the result to Firebase
    await writeToFirebase(DB_PATHS.DUELLO_SON_MAC, finalData);
    console.log('✅ Duello Son Maç stats written to Firebase');
...
async function updateDuelloSezonStats() {
  try {
    ...
    if (!results || !results.rows || !results.rows.length) {
      ...
      // Create an empty structure if no data
      const emptyData = { playerRows: [], playerCols: [], duels: {} };
      await writeToFirebase(DB_PATHS.DUELLO_SEZON, emptyData);
      console.log('✅ Empty Duello Sezon stats written to Firebase');
      return;
    }
    ...
    // Write the result to Firebase
    await writeToFirebase(DB_PATHS.DUELLO_SEZON, finalData);
    console.log('✅ Duello Sezon stats written to Firebase');
...
async function updatePerformanceGraphsStats() {
  try {
    ...
    if (!results || !results.rows || !results.rows.length) {
      ...
      await writeToFirebase(DB_PATHS.PERFORMANCE_DATA, emptyData);
      console.log('✅ Empty Performance Graphs data written to Firebase');
      return;
    }
    ...
    // Write the result to Firebase
    await writeToFirebase(DB_PATHS.PERFORMANCE_DATA, finalData);
    console.log('✅ Performance Graphs data written to Firebase');
